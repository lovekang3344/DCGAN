...
os.makedirs('./runs/MNIST', exist_ok=True)
manualSeed = 999
...
# test_loader = DataLoader(test_file, batch_size=batch_size, shuffle=True)
...
np.savetxt('./checkpoints/MNIST/G_losses.txt', G_losses)
np.savetxt('./checkpoints/MNIST/D_losses.txt', D_losses)
torch.save(netG.state_dict(), './checkpoints/MNIST/netG.pt')
torch.save(netD.state_dict(), './checkpoints/MNIST/netD.pt')
